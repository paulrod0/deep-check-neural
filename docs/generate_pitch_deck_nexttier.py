#!/usr/bin/env python3
"""
Deep-Check Investor Pitch Deck — PERSONALIZED FOR NEXTTIER VC
Pre-Seed Round · For Roger Almuni · April 2026

Variation of generate_pitch_deck.py with:
  - cover badge personalized to "For Roger Almuni · Nexttier VC"
  - footer says the deck was prepared specifically for Nexttier
  - closing slide includes a "Why this might fit Nexttier" note

The body content is identical to the master deck — same product story,
same numbers — so any update to the master should be ported here.
"""
...
OUTPUT_PATH = os.path.join(
    OUTPUT_DIR, "financing", "deep_check_pitch_deck_nexttier.pdf"
)
# Personalization for the recipient
RECIPIENT_NAME = "Roger Almuni"
RECIPIENT_FUND = "Nexttier VC"
DECK_DATE_LABEL = "Abril 2026"
...
def draw_bottom_bar(c, text="deep-check.ai", page_num=None, total=10):
...
def slide_cover(c):
    ...
    c.drawString(MARGIN, H - 200, "Pre-Seed Round")

    c.setFillColor(GREEN)
    c.setFont("Helvetica-Bold", 28)
    c.drawString(MARGIN, H - 240, "150.000 EUR")
    ...
    c.drawString(
        MARGIN, 55,
        f"Preparado para {RECIPIENT_NAME} · {RECIPIENT_FUND}  |  {DECK_DATE_LABEL}",
    )
    ...
    # Personalized recipient badge (replaces the generic model-version chip)
    badge_w, badge_h = 200, 36
    badge_x = W - MARGIN - badge_w
    badge_y = H - 116
    draw_card(c, badge_x, badge_y, badge_w, badge_h, bg=CARD_BG)
    c.setFillColor(GREEN)
    c.setFont("Helvetica-Bold", 9)
    c.drawCentredString(badge_x + badge_w / 2, badge_y + badge_h - 13,
                        f"PERSONALIZED FOR {RECIPIENT_FUND.upper()}")
    c.setFillColor(WHITE)
    c.setFont("Helvetica", 9)
    c.drawCentredString(badge_x + badge_w / 2, badge_y + 10,
                        f"{RECIPIENT_NAME} · pre-seed conversation")
...
def slide_solution(c):
    ...
    # 6 layers as cards in 2 rows of 3
    layers = [
    ...
    for i, (name, desc, weight) in enumerate(layers):
        ...
        c.drawString(x + 10, y + card_h - 20, name)
        c.setFillColor(GREEN_DARK if weight != "NEW" else HexColor("#ff9d00"))
        c.setFont("Helvetica-Bold", 9)
        c.drawRightString(x + card_w - 10, y + card_h - 20, weight)
        c.setFillColor(GRAY_LIGHT)
        c.setFont("Helvetica", 8)
        for j, line in enumerate(desc.split("\n")):
            c.drawString(x + 10, y + card_h - 38 - j * 11, line)

    # Key differentiators on the right
    diff_x = W / 2 + 30
    diff_y = H - 170

    diffs = [
        ("100% Client-Side",
         "Todo el procesamiento biometrico corre en el\nnavegador via WebAssembly. Cero datos al servidor."),
        ("GDPR Native",
         "Privacy-by-design. Sin almacenamiento biometrico.\nSin consentimiento complejo necesario."),
        ("6 Capas Independientes",
         "Fusion bayesiana en espacio logit.\nCada capa vota independientemente."),
        ("Solo Navegador",
         "Sin apps nativas. Sin SDKs pesados.\nFunciona en cualquier navegador moderno."),
    ]
    ...
    for i, (title, desc) in enumerate(diffs):
        ...
        c.drawString(diff_x, y, title)
        c.setFillColor(GRAY_LIGHT)
        c.setFont("Helvetica", 9)
        for j, line in enumerate(desc.split("\n")):
            c.drawString(diff_x, y - 14 - j * 12, line)
...
def slide_round(c):
    ...
    draw_bg(c)
    draw_top_bar(c)
    draw_bottom_bar(c, page_num=9)

    c.setFillColor(GREEN)
    c.setFont("Helvetica-Bold", 10)
    c.drawString(MARGIN, H - 40, "08")
    c.setFont("Helvetica", 10)
    c.drawString(MARGIN + 20, H - 40, "LA RONDA")

    c.setFillColor(WHITE)
    c.setFont("Helvetica-Bold", 32)
    c.drawString(MARGIN, H - 78, "Pre-Seed: 150.000 EUR")

    # Use of funds
    c.setFillColor(WHITE)
    c.setFont("Helvetica-Bold", 14)
    c.drawString(MARGIN, H - 115, "Uso de Fondos")

    funds = [
        ("Equipo", "50%", 0.50, GREEN),
        ("Certificaciones (iBeta)", "15%", 0.15, GREEN_DIM),
        ("Infraestructura ML", "15%", 0.15, BLUE_ACCENT),
        ("Go-to-Market", "15%", 0.15, HexColor("#ff9d4d")),
        ("Legal / Ops", "5%", 0.05, GRAY_MID),
    ]

    y = H - 150
    for label, pct_str, pct, color in funds:
        c.setFillColor(color)
        c.circle(MARGIN + 6, y + 4, 5, fill=True, stroke=False)
        c.setFillColor(GRAY_LIGHT)
        c.setFont("Helvetica", 11)
        c.drawString(MARGIN + 18, y, label)
        c.setFillColor(WHITE)
        c.setFont("Helvetica-Bold", 11)
        c.drawRightString(MARGIN + 220, y, pct_str)
        # Bar
        bar_x_start = MARGIN + 230
        bar_w = 150
        c.setFillColor(GRAY_DARK)
        c.roundRect(bar_x_start, y - 1, bar_w, 10, 5, fill=True, stroke=False)
        c.setFillColor(color)
        c.roundRect(bar_x_start, y - 1, bar_w * pct / 0.50, 10, 5, fill=True, stroke=False)
        y -= 30
    ...
    c.setFillColor(WHITE)
    c.setFont("Helvetica-Bold", 14)
    c.drawString(ms_x, H - 115, "Milestones 18 Meses")

    milestones = [
        ("Q2-Q3 2026", "Certificacion iBeta (ISO 30107-3)"),
        ("Q3-Q4 2026", "3-5 pilotos enterprise activos"),
        ("Q4 2026", "ARR 200K EUR"),
        ("Q1 2027", "Ronda Seed: 1.5 - 2M EUR"),
    ]

    y = H - 150
    for i, (timeline, milestone) in enumerate(milestones):
        draw_card(c, ms_x, y - 8, ms_w, 42, bg=CARD_BG)
        dot_x = ms_x + 12
        dot_y = y + 8
        c.setFillColor(GREEN)
        c.circle(dot_x, dot_y, 4, fill=True, stroke=False)
        if i < len(milestones) - 1:
            c.setStrokeColor(GRAY_DARK)
            c.setLineWidth(1)
            c.line(dot_x, dot_y - 6, dot_x, dot_y - 34)
        c.setFillColor(GREEN)
        c.setFont("Helvetica-Bold", 9)
        c.drawString(ms_x + 25, y + 18, timeline)
        c.setFillColor(GRAY_LIGHT)
        c.setFont("Helvetica", 10)
        c.drawString(ms_x + 25, y + 2, milestone)
        y -= 50

    # Seed round preview
    seed_y = 38
    draw_card(c, MARGIN, seed_y, W - 2 * MARGIN, 45, bg=HexColor("#0a1a12"))
    c.setFillColor(GREEN)
    c.setFont("Helvetica-Bold", 10)
    c.drawString(MARGIN + 15, seed_y + 26, "SIGUIENTE RONDA")
    c.setFillColor(WHITE)
    c.setFont("Helvetica-Bold", 14)
    c.drawString(MARGIN + 15, seed_y + 6, "Seed: 1.5 - 2M EUR")
    c.setFillColor(GRAY_LIGHT)
    c.setFont("Helvetica", 10)
    c.drawString(MARGIN + 200, seed_y + 6,
                 "Objetivo: Q1 2027  |  Con iBeta + pilotos + ARR demostrado")


def slide_why_nexttier(c):
    """Slide personalizado: por qué creo que esto puede encajar con Nexttier."""
    draw_bg(c)
    draw_top_bar(c, GREEN)
    draw_bottom_bar(c, page_num=10)

    # Section label
    c.setFillColor(GREEN)
    c.setFont("Helvetica-Bold", 11)
    c.drawString(MARGIN, H - 50, "10  ·  WHY THIS MIGHT FIT NEXTTIER")

    # Title
    c.setFillColor(WHITE)
    c.setFont("Helvetica-Bold", 32)
    c.drawString(MARGIN, H - 90, f"Roger, esto es por qué te lo mando.")

    # Subtitle
    c.setFillColor(GRAY_LIGHT)
    c.setFont("Helvetica", 14)
    c.drawString(MARGIN, H - 115,
                 "Cuatro razones concretas. Si alguna no encaja con vuestra tesis, lo entiendo.")

    # 4 reasons cards
    reasons = [
        ("Producto vivo, no slide",
         "Plataforma en producción desde marzo 2026 con cliente",
         "anual firmado pagando 3.200 EUR/mes y +50% MoM. No",
         "estoy buscando dinero para validar idea — busco capital",
         "para acelerar lo que ya funciona."),
        ("Ventana regulatoria irrepetible",
         "AI Act Art. 50 entra en vigor el 2 de agosto de 2026",
         "con multa de hasta 15M EUR / 3% facturación global.",
         "Cualquiera que opere en UE va a necesitar esto.",
         "El tiempo de mercado es ahora, no en 2027."),
        ("España + UE como ventaja, no excusa",
         "Infraestructura íntegramente europea (AWS Irlanda +",
         "Vercel UE). Cero transferencia de datos biométricos a",
         "EE.UU. La mayoría de competidores son americanos —",
         "para banca, sanidad y AAPP esto es decisivo."),
        ("Founder técnico con tracción real",
         "213k LOC, 9 modelos propios entrenados, 0 dependencias",
         "de APIs externas. Pipeline de ventas vivo (IE University,",
         "EUS Energía, cliente confidencial). El tipo de perfil que",
         "hace falta para resolver esto sin pedir 5M en seed."),
    ]

    card_w = (W - 2 * MARGIN - 20) / 2
    card_h = 145
    for i, (title, l1, l2, l3, l4) in enumerate(reasons):
        col = i % 2
        row = i // 2
        x = MARGIN + col * (card_w + 20)
        y = H - 145 - 20 - row * (card_h + 15) - card_h
        draw_card(c, x, y, card_w, card_h, bg=CARD_BG)
        # Number circle
        c.setFillColor(GREEN)
        c.circle(x + 25, y + card_h - 25, 11, fill=True, stroke=False)
        c.setFillColor(BG)
        c.setFont("Helvetica-Bold", 12)
        c.drawCentredString(x + 25, y + card_h - 28, str(i + 1))
        # Title
        c.setFillColor(WHITE)
        c.setFont("Helvetica-Bold", 13)
        c.drawString(x + 50, y + card_h - 22, title)
        # Body
        c.setFillColor(GRAY_LIGHT)
        c.setFont("Helvetica", 10)
        for li, line in enumerate([l1, l2, l3, l4]):
            c.drawString(x + 18, y + card_h - 50 - li * 16, line)

    # Bottom line
    c.setFillColor(GREEN_DIM)
    c.setFont("Helvetica-Oblique", 11)
    c.drawCentredString(W / 2, 30,
                        "Si me das 20 minutos te enseño el producto en vivo. — Pablo")


def slide_closing(c):
    """Page 11: Cierre personalizado para Roger / Nexttier."""
    draw_bg(c)
    ...
    c.drawCentredString(
        W / 2, 30,
        f"Confidencial  |  Preparado para {RECIPIENT_NAME} · {RECIPIENT_FUND}  |  "
        f"{DECK_DATE_LABEL}  |  Pre-Seed 150K EUR",
    )
...
def main():
    # Make sure the output directory exists (financing/)
    os.makedirs(os.path.dirname(OUTPUT_PATH), exist_ok=True)

    c = canvas.Canvas(OUTPUT_PATH, pagesize=landscape(A4))
    c.setTitle(f"Deep-Check · Pitch Deck · For {RECIPIENT_NAME} · {RECIPIENT_FUND}")
    c.setAuthor("Pablo Lopez Rodriguez")
    c.setSubject(f"Pre-Seed Investment Round · 150K EUR · For {RECIPIENT_FUND}")
