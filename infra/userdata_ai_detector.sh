#!/bin/bash
# =============================================================================
# userdata_ai_detector.sh — EC2 user-data for AI Image Detector training
# =============================================================================
# This script runs automatically when the EC2 instance boots.
# It sets up the environment, downloads data, and trains the model.
# =============================================================================
set -euo pipefail
exec > /var/log/deep-check-ai-detector-train.log 2>&1

echo "=============================================="
echo "  Deep-Check AI Image Detector Training"
echo "  Started: $(date)"
...
# Try Deep Learning AMI conda first, fall back to system python
...
mkdir -p /data/output/ai_image_detector
aws s3 cp "s3://${S3_BUCKET}/scripts/train_ai_image_detector.py" \
    /data/train_ai_image_detector.py \
    --region "$REGION" || {
...
mkdir -p /data/ai_image_detector/raw
mkdir -p /data/output/ai_image_detector

# ── 5. Start training in tmux ────────────────────────────────────────────
echo "[5/6] Starting training in tmux session: ai_detector_train"
tmux new-session -d -s ai_detector_train 2>/dev/null || true
tmux send-keys -t ai_detector_train \
    "python3 /data/train_ai_image_detector.py \
        --epochs 100 \
        --batch-size 32 \
        --lr 3e-4 \
        --output ai_image_detector_v1.onnx \
    2>&1 | tee /data/output/ai_image_detector/train.log" Enter
...
# Create a script that monitors training and uploads results when done
cat > /data/monitor_and_upload.sh << 'MONITOR_EOF'
#!/bin/bash
set -euo pipefail
S3_BUCKET="deep-check-models"
REGION="eu-west-1"
OUT_DIR="/data/output/ai_image_detector"

echo "Monitoring training completion..."
# Wait for training to finish (check for ONNX file or history completion)
while true; do
    if [ -f "${OUT_DIR}/meta.json" ]; then
        echo "Training complete! Uploading final results..."
        aws s3 sync "${OUT_DIR}/" "s3://${S3_BUCKET}/ai_image_detector/" \
            --region "$REGION" \
            --exclude "*.tmp" \
            --exclude "__pycache__/*"
    ...
    # Upload intermediate checkpoints every 10 minutes
    if [ -f "${OUT_DIR}/history.jsonl" ]; then
        aws s3 cp "${OUT_DIR}/history.jsonl" \
            "s3://${S3_BUCKET}/ai_image_detector/history.jsonl" \
            --region "$REGION" 2>/dev/null || true
    fi
    sleep 600
done

echo "=== AI Image Detector Training Complete: $(date) ==="
...
echo "  Monitor: tmux attach -t ai_detector_train"
echo "  GPU:     watch -n5 nvidia-smi"
echo "  Log:     tail -f /data/output/ai_image_detector/train.log"
echo "=============================================="
