#!/usr/bin/env python3
"""V9.4 WARM RESTART -- Load best checkpoint, fresh LR, continue training."""
...
@dataclass
class C:
    SZ:int=224; BS:int=64; EPOCHS:int=40
    LR_BB:float=1e-5; LR_HEAD:float=1e-4
    WD:float=0.01; EMA_DECAY:float=0.999
    UNFREEZE_BLOCKS:int=4; GRAD_CLIP:float=1.0; WORKERS:int=4
    DATA:Path=Path("/home/ubuntu/data"); OUT:Path=Path("/home/ubuntu/training/v9_warm")
    CHECKPOINT:Path=Path("/home/ubuntu/training/v9_simple/best_v9_simple.pt")
    S3_METRICS:str="s3://deep-check-models/training/v9_warm/metrics.jsonl"
    S3_BEST:str="s3://deep-check-models/deepfake/v9/best_v9_warm.pt"
...
def main():
    ...
    ckpt_auc=ckpt.get("val_auc",0); ckpt_eer=ckpt.get("val_eer",1)
    log.info(f"Checkpoint: AUC={ckpt_auc:.4f}, EER={ckpt_eer:.4f}")
    ...
    train_loader=DataLoader(train_ds,batch_size=cfg.BS,sampler=sampler,num_workers=cfg.WORKERS,pin_memory=True,drop_last=True)
    val_loader=DataLoader(val_ds,batch_size=cfg.BS,num_workers=cfg.WORKERS,pin_memory=True)
    ...
    if "ema_state" in ckpt:
        try:
            for k in ema.shadow:
                if k in ckpt["ema_state"]:
                    ema.shadow[k]=ckpt["ema_state"][k].to(device)
            log.info("EMA loaded")
        except: log.info("EMA load failed, fresh EMA")
    ...
    optimizer=torch.optim.AdamW([{"params":bb_p,"lr":cfg.LR_BB},{"params":head_p,"lr":cfg.LR_HEAD}],weight_decay=cfg.WD)
    scheduler=torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer,T_0=10,T_mult=2,eta_min=1e-7)
    scaler=torch.amp.GradScaler("cuda")
    best_auc=ckpt_auc; best_eer=ckpt_eer; best_epoch=0

    sep="="*60
    log.info(f"\n{sep}")
    log.info(f"V9.4 WARM RESTART from AUC={ckpt_auc:.4f} EER={ckpt_eer:.4f}")
    log.info(f"LR bb={cfg.LR_BB} head={cfg.LR_HEAD}, CosineWarmRestarts T0=10")
    log.info(sep)
    ...
    for epoch in range(1,cfg.EPOCHS+1):
        ...
        log.info(f"[WR-E{epoch:02d}] val_auc={va:.4f} val_eer={ve:.4f} train_auc={ta:.4f} gap={gap:.4f} lr={lr:.2e} time={elapsed:.0f}s")
        metric={"epoch":epoch,"phase":"WARM_RESTART","model":"v9.4-warm",
                "train_loss":train_m["loss"],"train_auc":ta,
                "val_loss":val_m["loss"],"val_auc":va,"val_eer":ve,"val_acc":val_m["acc"],
                "lr":lr,"gap":gap,"time_s":elapsed}
        ...
        if va>best_auc:
            ...
            torch.save({"epoch":epoch,"model_state":model.state_dict(),"ema_state":ema.shadow,
                        "val_auc":va,"val_eer":ve},cfg.OUT/"best_v9_warm.pt")
            log.info(f"  * New best AUC={best_auc:.6f} EER={best_eer:.4f}")
            try: os.system(f"aws s3 cp {cfg.OUT/'best_v9_warm.pt'} {cfg.S3_BEST} --quiet")
            except: pass
        if epoch>20 and epoch-best_epoch>15: log.info(f"Early stopping (best={best_epoch})"); break
